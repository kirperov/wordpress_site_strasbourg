...
if ( ! class_exists( 'Kirki_Output_Field_Typography' ) ) {
	...
	class Kirki_Output_Field_Typography extends Kirki_Output {
		...
		protected function process_output( $output, $value ) {
			...
			$output['mobile_media_query'] = 'global';
			...
			$fontSizeMedia = $output['media_query'];
			if ( isset( $value['mobile-font-size'] ) && ! empty( $value['mobile-font-size'] ) ) {
				$this->styles[ $output['mobile_media_query'] ][ $output['element'] ]['font-size'] = $value['mobile-font-size'];
				$fontSizeMedia = "@media only screen and (min-width: 768px)";
			}
			// Take care of font-size.
			if ( isset( $value['font-size'] ) && ! empty( $value['font-size'] ) ) {
				$this->styles[ $fontSizeMedia ][ $output['element'] ]['font-size'] = $value['font-size'];
            ...
            foreach ($this->output as $index => $o) {
					if(!isset($o['property']) || !is_array($value)){
						continue;
					}

					$v = parent::apply_value_pattern($o, $value);
					if(!is_array($v)){
						$o['media_query'] = (isset($o['media_query'])) ? $o['media_query'] : 'global';
						$o['element']     = (isset($o['element'])) ? $o['element'] : 'body';
						$this->styles[ $o['media_query'] ][$o['element']][ $o['property'] ] = $v;
					}
