...
if ( ! class_exists( 'Kirki_Controls_Repeater_Control' ) ) {
	...
	class Kirki_Controls_Repeater_Control extends Kirki_Customize_Control {
		...
		protected function render_content() {
			...
			<?php if ( 0 && isset( $this->choices['limit'] ) ) : ?>
					...
					<div class="repeater-row-content">
						<# _.each( data, function( field, i ) { #>
							...
							<div class="repeater-field repeater-field-{{{ field.type }}}">
								...
								<# } else if ( 'color' === field.type ) { #>
							        ...
							        <label>
							            ...
							            <# if ( field.description ) { #>
							                <span class="description customize-control-description">{{{ field.description }}}</span>
							            <# } #>
							            <input data-alpha="{{{ field.alpha }}}" class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr( $this->l10n['hex-value'] ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
								...
								<# } else if( 'sectionseparator' === field.type ) { #>
