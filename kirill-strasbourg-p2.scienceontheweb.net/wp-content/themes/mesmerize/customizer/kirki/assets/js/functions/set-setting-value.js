function kirkiSetSettingValue(setting, value) {
    ...
    if ('kirki-checkbox' === controlType || 'kirki-switch' === controlType || 'kirki-toggle' === controlType) {
        ...
        if (alphaColorControl.data('spectrum.id') === undefined) {
            alphaColorControl.wpColorPicker('color', value);
        } else {
            alphaColorControl.spectrum("set", value);
        }
        ...
        if (undefined !== value['font-family']) {
            ...
            if ('undefined' !== typeof $select && $select.length) {
                selectize = $select[0].selectize;
        ...
        if (undefined !== value['text-transform']) {

            $select = jQuery(wp.customize.control(setting).container.find('.text-transform select')).selectize();
            if ('undefined' !== typeof $select && $select.length) {
                ...
                // Update the value visually in the control
                selectize.setValue(value['text-transform'], true);
            }
        }
        ...
        if (undefined !== value['mobile-font-size']) {

            // Update the value visually in the control
            jQuery(wp.customize.control(setting).container.find('.mobile-font-size input')).prop('value', value['mobile-font-size']);
        ...
        if (undefined !== value.color) {
            ...
            if (typographyColor.data('spectrum.id') === undefined) {
                typographyColor
                    .attr('data-default-color', value.color)
                    .data('default-color', value.color)
                    .wpColorPicker('color', value.color);
            } else {
                typographyColor.spectrum("set", value);
            }
        ...
        // Do nothing
        jQuery(wp.customize.control(setting).container.find('input')).each(function () {
            var $input = jQuery(this);
            var position = ($input.parent().attr('class').match(/top|left|right|bottom/) || []).pop();

            if (position && value[position] !== undefined) {
                $input.prop('value', value[position]);
            }
